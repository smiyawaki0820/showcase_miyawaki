...
def evaluate_multiclass_without_none(model, data_test, len_test,
    ...
    for xss, yss, sent_id, file_name in tqdm(data_test, total=len_test, mininterval=5):
        ...
        temp = F.log_softmax((torch.ones(yss.size()[0], yss.size()[1], 4)*0.25).cuda())
    ...
    writer = csv.writer(open('./result/model-'+model_id+'_result.csv', 'a'))
    writer.writerow([f])
...
def calc_best_thres(best_result, results, thres_lists, labels, model_id):
    ...
    index = ['ga', 'wo', 'ni', 'all']
    with open('./result/model-'+model_id+'_result.csv', 'a') as csv_file:
