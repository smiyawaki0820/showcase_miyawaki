...
class NtcBucketIterator(PaddingBucketIterator):  # sub_class(base_class)
    """Generate padded mini-batches to minimize padding as much as possible about NAIST Text Corpus."""
    def __init__(self, dataset: List, batch_size=128, shuffle=False, decode: bool = False):
        self.decode = decode
        super(NtcBucketIterator, self).__init__(                                # base_class の __init__ を呼び出し
            self.get_dataset(dataset), self.sort_key, batch_size, shuffle)
        ...
        for sentence in dataset:
            if sentence["pas"]:
                ...
                for pas in pas_seq:
                    ...
                    if self.decode:
                        ys = [gold, p_id, sentence["sentence id"], sentence["file name"]]
                    else:
                        ys = gold
...
# data_train, data_dev = list(end2end...)
def end2end_single_seq_instance(data, batch_generator):
    for i, sentence in enumerate(data):
        #if i <= 0: print('sent: ', sentence)
        if sentence["pas"]:
            instances = list(batch_generator(sentence))
            # batch_generator で xss, yss に分化
            xss, yss, sent_id, file_name = zip(*instances)
            yield [list(map(lambda e: torch.stack(e), zip(*xss))), torch.stack(yss), sent_id, file_name]
...
def e2e_single_seq_sentence_batch(instance):
    ...
    for pas in pas_seq:
        ...
        ts = torch.LongTensor([int(t) for t in tokens])
        ps = torch.Tensor([[1.0] if i == p_id else [0.0] for i in range(len(tokens))])
        yield [[ts, ps], ys, sent_id, file_name]
...
def end2end_single_seq_instance_with_prev_sentences(data):
    for sentence in data:
        if sentence["curr"]["pas"]:
            ...
            xss, yss, sent_id, file_name = zip(*instances)
            yield [p_wss, list(map(lambda e: torch.stack(e), zip(*xss))), torch.stack(yss), sent_id, file_name]


def e2e_single_seq_batch_generator(instances, num_instances, batch_size):
    for i in range(num_instances):
        start = i * batch_size
        xss, yss, sent_id, file_name = zip(*instances) #[start:start + batch_size])
        tss, pss = zip(*xss)
        print("tss", torch.stack(tss))
        print("pss", pss)
        print("yss", yss)
        yield [torch.stack(tss), torch.stack(pss)], torch.stack(yss), sent_id, file_name
...
def pretrained_pos_vecs(data_path, index_file_name, size_e):
    wIdx = {}
    lexicon_size = 0
    wIdxData = open(data_path + index_file_name)
    for line in wIdxData:
    ...
    vocab_size = 0
    # wVecData = open("{0}/word_vec_{1}.txt".format(data_path, size_e))
    wVecData = open("{0}/pos_vec-{1}-jawiki20160901.txt".format(data_path, size_e))
    matrix = np.random.uniform(-0.05, 0.05, (lexicon_size, size_e))
    for line in wVecData:
        values = line.rstrip().split(" ")
        word = values[0]
